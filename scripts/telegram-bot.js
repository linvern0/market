...
async function handleAdminCommand(db, token, settingsRef, admins, chatId, cmd, argStr) {
  ...
  if (cmd === '/adminlar') {
    ...
    admins.forEach((a) => { text += `• ${esc(a.name || 'Admin')} — Chat ID: <code>${esc(a.chatId)}</code>\n`; });
    return sendMessage(token, chatId, text);
  }
  ...
  if (cmd === '/admin_qoshish') {
    ...
    await sendMessage(token, newChatId, `👋 Siz "${esc(shopName)}" do'konining Telegram admini etib tayinlandingiz. /yordam yozib buyruqlar ro'yxatini ko'ring.`).catch(() => {});
    return;
  }
