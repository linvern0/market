// telegram-bot.js
// ---------------------------------------------------------------------
// IKKI TOMONLAMA Telegram bot: GitHub Actions orqali muntazam (masalan
// har 5 daqiqada) ishga tushib, botga kelgan YANGI xabarlarni (Telegram
// Bot API'ning getUpdates usuli orqali) o'qiydi va ularga javob
// qaytaradi. telegram-notify.js skriptidan farqli — bu FAQAT bir
// tomonlama ("push") xabar emas, balki odamlar botga yozganda ularga
// javob beradi:
//
// 1) Har qanday ADMIN (Sozlamalar > Telegram bo'limida qo'shilgan Chat
//    ID) botga buyruq (masalan /qarzlar, /tolov, /statistika) yozib,
//    ilovani to'liq boshqarishi mumkin.
// 2) Har qanday MIJOZ (qarzdor) botga /start yozib, telefon raqamini
//    yuborsa (yoki admin oldindan Telegram username'ni yozib qo'ygan
//    bo'lsa), tizim uni "debtors" kolleksiyasidagi mos yozuvga bog'laydi
//    — shundan keyin unga YANGI qarz va TO'LOV haqida xabarlar avtomatik
//    boradi (buni ilovaning o'zi, index.html, qarz/tolov amalga
//    oshganda darhol yuboradi).
//
// Bu skript "offset" (oxirgi ko'rilgan update_id) qiymatini
// Firestore'dagi dokon/telegramBotState hujjatida saqlaydi — shu orqali
// har ishga tushganda faqat YANGI xabarlarni qayta ishlaydi (eskilarini
// takrorlamaydi).
// ---------------------------------------------------------------------
...
function initFirebase() {
  ...
  if (!raw) {
    throw new Error(
      "FIREBASE_SERVICE_ACCOUNT topilmadi. GitHub repo -> Settings -> " +
      "Secrets and variables -> Actions bo'limiga xizmat hisobi " +
      "(service account) JSON kalitini FIREBASE_SERVICE_ACCOUNT nomi " +
      "bilan qo'shing."
    );
  }
...
// Telefon raqamlarni solishtirish uchun: faqat raqamlarni qoldiramiz va
// oxirgi 9 ta raqamni (O'zbekiston mobil raqami davlat kodisiz) tayanch
// sifatida olamiz — shu orqali "+998901234567", "998901234567",
// "90 123 45 67" kabi turli formatlar bir xil deb topiladi.
function phoneKey(phone) {
  const digits = String(phone || '').replace(/\D/g, '');
  return digits.slice(-9);
}

// Telegram username'larni solishtirish uchun: boshidagi "@" olib
// tashlanadi va kichik harflarga o'giriladi ("@AzizKarimov" va
// "azizkarimov" bir xil deb topiladi).
...
// Telegram chat oynasining pastidagi "Menu" tugmasi faqat shu API
// chaqirilgan bo'lsa buyruqlar ro'yxatini ko'rsatadi. Hozirgacha bu hech
// qayerda chaqirilmagani uchun menyu bo'sh ko'rinardi. Mijozlar uchun
// umumiy (default) ro'yxat, har bir admin uchun esa o'sha chatga xos
// (scope: chat) to'liqroq ro'yxat o'rnatiladi.
const CUSTOMER_COMMANDS = [
  { command: 'start', description: "Botni boshlash / ro'yxatdan o'tish" },
  { command: 'qarzim', description: "Joriy qarzimni ko'rish" },
];

const ADMIN_COMMANDS = [
  { command: 'qarzlar', description: "Faol qarzlar ro'yxati" },
  { command: 'qarz', description: 'Mijoz qarzini qidirish' },
  ...
  { command: 'mahsulot', description: "Tovar narxi/qoldig'ini ko'rish" },
  { command: 'tovar_narx', description: 'Tovar narxini yangilash' },
  { command: 'adminlar', description: "Admin ro'yxati" },
  { command: 'admin_qoshish', description: "Yangi admin qo'shish" },
  { command: 'admin_ochirish', description: "Adminni o'chirish" },
  { command: 'yordam', description: 'Yordam' },
];

async function setupCommandMenus(token, admins) {
  try {
    await tgCall(token, 'setMyCommands', { commands: CUSTOMER_COMMANDS });
    ...
    console.error("setMyCommands o'rnatishda xatolik:", e);
  }
}
...
// Bir nechta joyda ishlatiladigan umumiy funksiya: topilgan qarzdor
// yozuv(lar)iga shu Telegram chat ID'ni bog'laydi va "xush kelibsiz" +
...
// Admin qarzdorga Telegram username'ni oldindan yozib qo'ygan bo'lsa
// (Qarzdor formasidagi "Telegram username" maydoni), mijoz botga
// /start bosishi bilan, telefon raqami so'ralmasdan, avtomatik
// tanilishi va qarzi haqida ma'lumot olishi kerak — shu funksiya buni
// amalga oshiradi.
...
async function handleAdminCommand(db, token, settingsRef, admins, chatId, cmd, argStr) {
  ...
  if (cmd === '/tolov') {
    ...
    await sendMessage(
      token,
      chatId,
      `✅ ${fmt(applied)} so'm to'lov qabul qilindi: ${esc(d.debtorName || d.org || '')}${
    ...
    if (d.debtorId) {
      ...
      if (cid) {
        await sendMessage(
          token,
          cid,
          `💵 <b>${esc(shopName)}</b>\n\nSizning ${fmt(applied)} so'm to'lovingiz qabul qilindi.${
  ...
  if (cmd === '/qarz_qoshish') {
    ...
    const debt = {
      ...
      paidDate: null,
      date: now,
      note: note || "Telegram bot orqali qo'shildi",
      seller: 'Telegram admin',
      type: 'person',
    };
  ...
  if (cmd === '/admin_qoshish') {
    ...
    if (!newChatId || !/^-?\d+$/.test(newChatId)) {
      return sendMessage(token, chatId, 'Foydalanish: /admin_qoshish 123456789 Ism');
    }
    if (admins.some((a) => String(a.chatId) === newChatId)) {
      return sendMessage(token, chatId, 'Bu Chat ID allaqachon admin.');
    }
    const newAdmin = { id: 'tg_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6), name, chatId: newChatId };
    await settingsRef.set({ telegramAdmins: admin.firestore.FieldValue.arrayUnion(newAdmin) }, { merge: true });
    await sendMessage(token, chatId, `✅ Yangi admin qo'shildi: ${esc(name)} (${esc(newChatId)})`);
    await sendMessage(
      token,
      newChatId,
      `👋 Siz "${esc(shopName)}" do'konining Telegram admini etib tayinlandingiz. /yordam yozib buyruqlar ro'yxatini ko'ring.`
    ).catch(() => {});
    return;
  }
  ...
  if (cmd === '/admin_ochirish') {
    ...
    if (!target) return sendMessage(token, chatId, 'Foydalanish: /admin_ochirish 123456789');
    const remaining = admins.filter((a) => String(a.chatId) !== target);
    if (remaining.length === admins.length) return sendMessage(token, chatId, "Bunday Chat ID admin ro'yxatida topilmadi.");
...
async function main() {
  ...
  if (!settingsSnap.exists) {
    console.log('Sozlamalar topilmadi.');
    return;
  }
  ...
  // Menyu ro'yxatini adminlar o'zgargandagina qayta o'rnatamiz (har safar
  // chaqiraverish shart emas, lekin xato bermaydi ham — shunchaki tejash
  // uchun).
  const adminsHash = admins.map((a) => a.chatId).filter(Boolean).sort().join(',');
  const savedHash = stateSnap.exists ? stateSnap.data().adminsCommandHash || '' : '';
  if (adminsHash !== savedHash) {
    await setupCommandMenus(token, admins);
    await stateRef.set({ adminsCommandHash: adminsHash }, { merge: true });
  }

  const updatesRes = await tgCall(token, 'getUpdates', { offset, timeout: 0, limit: 50 });
  if (!updatesRes.ok) {
    console.error('getUpdates xatosi:', updatesRes.description);
    return;
  }
  const updates = updatesRes.result || [];
  if (!updates.length) {
    console.log("Yangi xabar yo'q.");
    return;
  }
